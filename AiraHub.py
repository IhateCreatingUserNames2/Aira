...
@app.get("/connect/stream", tags=["SSE"])
async def connect_and_stream(
    req: Request,
    agent_url: str,
    name: str,
    aira_capabilities: str = "hybrid"
):
    capabilities = [c.strip().lower() for c in aira_capabilities.split(",")]
    ...
    await req.app.state.store.save_agent(ag)
    log.info(" SSE registered agent: %s (%s)", name, capabilities)

    async def sse_loop() -> AsyncGenerator[str, None]:
        while True:
            if await req.is_disconnected():
                log.warning(" SSE disconnected for agent: %s", name)
                ag.status = AgentStatus.OFFLINE
                await req.app.state.store.save_agent(ag)
                break
    ...
    return EventSourceResponse(sse_loop())

@app.post("/connect/stream/init", tags=["SSE"])
...
async def status(req: Request):
    ...
    detailed = []
    for a in agents:
        source = "sse" if "streamed" in a.tags else "manual"
        heartbeat_ago = now - a.last_seen
        detailed.append({
            "name": a.name,
            "url": a.url,
            "status": a.status,
            "source": source,
            "aira_capabilities": a.aira_capabilities,
            "heartbeat_seconds_ago": round(heartbeat_ago, 1),
        })

    return {
        "uptime": round(now - req.app.state.started, 1),
