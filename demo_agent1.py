    ...
    async def _handle_tasks_send(self, params, req_id):
        ...
        if "parameters:" in text.lower() or "with parameters:" in text.lower():
            try:
                ...
                print(f"⚠️ Error parsing parameters JSON: {str(e)}")
                # Fall back to regex extraction - keep the existing code below

        # Simple parsing fallback - only if JSON parsing fails
        if not tool_params:
            for tool in self.tools:
                if tool.name.lower() in text.lower():
                    tool_name = tool.name
